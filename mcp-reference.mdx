...
Include your agent API key on every request:

```
Authorization: pam_mkey_<prefix>.<secret>
```

Some clients prepend `Bearer `. The server accepts both forms.

Generate keys on the [Memory MCP page](https://pam.harmix.ai/for-developers) in the PAM app — not via this endpoint.
