...
Передавайте API ключ агента в кожному запиті:

```
Authorization: pam_mkey_<prefix>.<secret>
```

Деякі клієнти додають префікс `Bearer `. Сервер приймає обидва формати.

Генеруйте ключі на [сторінці Memory MCP](https://pam.harmix.ai/for-developers) у застосунку PAM — не через цей endpoint.
